...
"	WHERE \"Order_id\"=?;";
    ...
    public void UpdateOrderAdmin(int order_id,String sender, String receiver, String originAddress, String destinationAddres, String sender_phone, String receiver_phone,int weight,String status,float total_cost,boolean payment_status) {
        String sql = "UPDATE public.Order SET \"Sender\"=?, \"Receiver\"=?, \"Origin_address\"=?,"
                + " \"Destination_address\"=?, \"Sender_phone\"=?, \"Receiver_phone\"=?,\"Weight\" = ? "
                + "\"Status\"=? \"Total Cost\"=?, \"Payment status\"=? \n" +
                    "WHERE \"Order_id\"=?;";
        try {
            psm = connection.prepareStatement(sql);
            psm.setString(1, sender);
            ...
            psm.setString(6, receiver_phone);
            psm.setInt(7, weight);
            psm.setString(8, status);
            psm.setFloat(9, total_cost);
            psm.setBoolean(10, payment_status);
            psm.setInt(11, order_id);
